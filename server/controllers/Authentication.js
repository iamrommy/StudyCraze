...
exports.setUserAccountType = async(req, res)=>{
    try{
        //fetch id
        ...
        userDetails.accountType = account_type;
        ...
        res.json({
            success: true,
            message: `User account is set to ${userDetails.accountType} successfully`,
            token: token,
            user: userDetails
        });
    }
    catch(error){
        console.log('Error in setting account type of user', error);
        return res.status(500).json({
            success:false,
            message:error.message
        });
